...
This repository now also includes a lightweight `backend/` service that can sit behind a WeChat mini-program:

- Pure HTTP API on port `3000`
- `GET /ping` health check
- Wide-open CORS for MVP integration
- Strictly serial PDF rendering with queue worker concurrency fixed at `1`
- Low-memory Puppeteer rendering for 1GB RAM VPS deployment
- PM2 process file at `ecosystem.config.js`
- Resume customization endpoint with SQLite job persistence
- Optional DeepSeek enhancement with heuristic fallback when no API key is set

Backend deployment notes live in [docs/deploy-wechat-backend-1gb-vps.md](docs/deploy-wechat-backend-1gb-vps.md).
...
- `GET /api/v1/resume/jobs`
- `GET /api/v1/resume/jobs/:jobId`
- `GET /api/v1/resume/jobs/:jobId/pdf`
...
The repository now includes a lightweight backend in `backend/` for a WeChat mini-program MVP.

There is now also a native WeChat mini-program client in `miniprogram/`, styled with an Apple-inspired visual system:

- black and light-gray section blocks
- restrained blue accents
- oversized typography
- pill buttons and generous spacing
- minimal card hierarchy

Core endpoints:

- `GET /ping`
- `POST /api/v1/wechat/login`
- `GET /api/v1/wechat/me`
- `GET /api/v1/billing/catalog`
- `GET /api/v1/billing/overview`
- `POST /api/v1/billing/orders`
- `POST /api/v1/billing/orders/:orderId/mock-pay`
- `POST /api/v1/resume/customize`
- `POST /api/v1/resume/customize/async`
- `GET /api/v1/resume/jobs/:jobId`
- `GET /api/v1/resume/jobs/:jobId/pdf`
- `POST /api/v1/pdf/render`
...
# miniprogramRoot is already configured as ./miniprogram
```

Before testing on a real device:

- replace `appid` in `project.config.json` with your real mini-program AppID
- add `https://api.myootdai.com` to the mini-program request legal domain list
- configure `WECHAT_APP_ID` and `WECHAT_APP_SECRET` on the backend to switch from mock login to real `jscode2session`
- the mini-program home screen now has three areas: resume builder, PDF records, and benefits/orders
- mock payment is enabled by default so the order and entitlement flow can be tested before real WeChat Pay is connected
...
Async resume customization example:

```bash
curl -X POST http://127.0.0.1:3000/api/v1/resume/customize/async \
  -H 'Content-Type: application/json' \
  -d '{
    "resumeMarkdown": "# Alex Chen\nBackend engineer with Node.js and API experience.",
    "job": {
      "companyName": "Acme AI",
      "roleTitle": "Backend Engineer",
      "description": "Need Node.js, Redis, PDF, deployment, and queue experience.",
      "language": "en"
    },
    "options": {
      "renderPdf": false
...
- If `WECHAT_APP_ID` and `WECHAT_APP_SECRET` are configured, `/api/v1/wechat/login` uses real `jscode2session`
- If they are not configured and `WECHAT_ALLOW_DEV_LOGIN=true`, the backend falls back to a development mock session so the mini-program can continue integration work
